/**
 * Pure investor ID generator.
 *
 * Generates session-scoped investor identifiers of the form `INV-YYYY-XXXXXX`,
 ...
 *
 * A near-copy of `kite-id-generator.ts`, swapping the `KITE` prefix for `INV`
 * and reusing the same unambiguous 32-char alphabet.
 *
...
/** Matches a well-formed investor ID: `INV-YYYY-XXXXXX`. */
export const INV_ID_PATTERN =
  /^INV-\d{4}-[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{6}$/;

/** Random number generator returning a float in the half-open interval [0, 1). */
export type Rng = () => number;

/**
 * Generate an investor ID.
 *
 ...
 * @returns A string of the form `INV-YYYY-XXXXXX`.
 */
export function generateInvestorId(
  rng: Rng = Math.random,
  year: number = new Date().getFullYear(),
): string {
  const length = INV_ID_ALPHABET.length;
  let suffix = '';
  for (let i = 0; i < 6; i++) {
